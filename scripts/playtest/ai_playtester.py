...
def playtest_task(scenario, task, max_attempts=3, language='English'):
    """
    ...
    # 1. Get NPC Greeting
    greeting_system = build_greeting_system_prompt(scenario, task, language=language, mood=mood)
    
    seed = [{'role': 'user', 'content': 'Hello.'}]
    try:
        raw_greeting = produce_greeting_turn(seed, greeting_system, speaker=scenario.speaker, max_sentences=GREETING_MAX_SENTENCES, actor_fn=call_actor)
        clean_greeting, _ = extract_and_format_vocab(raw_greeting, language, scenario)
    ...
    for attempt in range(max_attempts):
        ...
        # 3. Judge evaluates
        try:
            done, hint = evaluate_task(learner_msg, task.done_when, conversation, language)
            if done:
        ...
        if attempt < max_attempts - 1:
            actor_sys = build_actor_system_prompt(scenario, task, language=language, mood=mood)
            try:
                raw_npc_msg = produce_actor_turn(conversation, actor_sys, speaker=scenario.speaker, max_sentences=ACTOR_MAX_SENTENCES, actor_fn=call_actor)
                clean_npc_msg, _ = extract_and_format_vocab(raw_npc_msg, language, scenario)
