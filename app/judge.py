...
def judge_deterministic(user_input: str, done_when: str, language: str, vocab_targets=None):
    ...
    if lang in ('english', 'en'):
        ...
        #
        # IT MUST NOT RETURN NO. Japanese orthography alone defeats an
        # enumerated list — 処方箋/処方せん, おすすめ/お勧め/オススメ — before
        # synonyms are considered. Rejecting here would trade judge_llm's false
        # negatives for a larger set of our own, which is the failure this
        # project treats as worst. Measured, not assumed: BACKLOG OPEN-18.
        if vocab_targets:
