...
## Context
During initial scenario generation, `scripts/playtest/ai_playtester.py` was created as an experimental script to simulate multi-turn learner-actor dialogue using an LLM-driven learner persona (`LEARNER_SYS`). The script checks whether task goals are winnable against the task judge (`evaluate_task`). 
...
## Decision
Formalize and wire `scripts/playtest/ai_playtester.py` into the development and testing workflow:
1. **Tool Integration & Execution Wiring:** `scripts/playtest/ai_playtester.py` is wired via `make playtest` in [Makefile](file:///Users/pk/language-learning-app/Makefile) and unit-tested in [tests/test_playtester.py](file:///Users/pk/language-learning-app/tests/test_playtester.py).
...
- BL-19 is resolved and closed in `docs/BACKLOG.md`.
...
It stays manual, and deliberately so. `scripts/playtest/ai_playtester.py` drives the
...
- `scripts/playtest/playtest_sample.py` resumes from its `--out` file by design, so
