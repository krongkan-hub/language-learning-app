...
## Context
During initial scenario generation, `dev/playtest/ai_playtester.py` was created as an experimental script to simulate multi-turn learner-actor dialogue using an LLM-driven learner persona (`LEARNER_SYS`). The script checks whether task goals are winnable against the task judge (`evaluate_task`). 
...
## Decision
Formalize and wire `dev/playtest/ai_playtester.py` into the development and testing workflow:
1. **Tool Integration & Execution Wiring:** `dev/playtest/ai_playtester.py` is wired via `make playtest` in [Makefile](Makefile) and unit-tested in [dev/tests/test_playtester.py](dev/tests/test_playtester.py).
...
It stays manual, and deliberately so. `dev/playtest/ai_playtester.py` drives the
7B model through multi-turn dialogue, so it costs minutes per scenario and
cannot join `dev/check_all.sh`, which the project keeps under six seconds
...
- `dev/playtest/playtest_sample.py` resumes from its `--out` file by design, so
