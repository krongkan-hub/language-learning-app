...
from dev.playtest.ai_playtester import playtest_task, clean_msg
...
def test_ai_playtester_module_importable():
    from dev.playtest.ai_playtester import LEARNER_SYS, playtest_task
...
def test_ai_playtester_runs_single_turn_mock(monkeypatch):
    ...
    monkeypatch.setattr("dev.playtest.ai_playtester._llm_chat", mock_llm_chat)
    monkeypatch.setattr("dev.playtest.ai_playtester.call_actor", mock_call_actor)
